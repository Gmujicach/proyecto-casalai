...
class rusuarios extends BD{
	...
	function set_descripcion_ma($valor){
		$this->descripcion_ma = $valor; //fijencen como se accede a los elementos dentro de una clase
	...
	function generarPDF(){
		...
		try{
			...
			$resultado = $co->prepare("Select * from tbl_marcas where descripcion_ma like :descripcion_ma ");
			$resultado->bindValue(':descripcion_ma','%'.$this->descripcion_ma.'%');
			...
			$html .= "<h1>Listado de Marcas</h1></div>";
            $html .= "<table>";
            $html .= "<thead>";
			$html = $html."<thead>";
			$html = $html."<tr>";
			$html = $html."<th>Nombre de la Marca</th>";
			...
			if($fila){
				...
				foreach($fila as $f){
					$html = $html."<tr>";
					$html = $html."<td style='text-align:center'>".$f['descripcion_ma']."</td>";
