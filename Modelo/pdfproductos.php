...
class rusuarios extends datos2{
	...
	function generarPDF(){
		...
		try{
			...
			$resultado = $co->prepare("Select * from tbl_productos where codigo like :codigo and 
										nombre_p like :nombre ");
			...
			$html .= "<h1>Listado de Productos</h1></div>";
            $html .= "<table>";
            $html .= "<thead>";
			$html = $html."<thead>";
			$html = $html."<tr>";
			$html = $html."<th>Codigo del Producto</th>";
			$html = $html."<th>Nombre Producto</th>";
			...
			if($fila){
				...
				foreach($fila as $f){
					...
					$html = $html."<td style='text-align:center'>".$f['nombre_p']."</td>";
