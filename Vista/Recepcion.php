...
<style>
  /* Asegurar que el botón eliminar esté alineado en la misma fila */
  .grupo-producto {
    display: flex;
  ...
  .grupo-producto > div {
    /* Que cada columna tenga la misma altura, para alinear el botón bien */
    display: flex;
    flex-direction: column;
  }

  /* Botón eliminar al final, con margen para que quede separado */
  .btn-eliminar-producto {
    margin-top: 25px; /* Para que quede alineado con inputs */
    white-space: nowrap;
  }
...
$(document).on('click', '.btn-modificar', function (e) {
  ...
  productos.forEach(item => {
    html += `
    <div class="grupo-producto row mb-2">
      <div class="col-md-6">
        <label>Producto</label>
        <select class="form-control" name="productos[]">
          ${productosDisponibles.map(prod => `
            <option value="${prod.id_producto}" ${prod.id_producto == item.id_producto ? 'selected' : ''}>
      ...
      <div class="col-md-2">
        ...
        <input type="number" class="form-control" name="cantidades[]" value="${item.cantidad}" min="1">
      </div>
      <div class="col-md-2">
        <label>Costo</label>
        <input type="number" class="form-control" name="costos[]" value="${item.costo}" min="0" step="0.01">
        <input type="hidden" name="iddetalles[]" value="${item.iddetalles || ''}">
      </div>
      <div class="col-md-1 d-flex align-items-end">
        <button type="button" class="btn btn-danger btn-eliminar-producto">Eliminar</button>
      </div>
    </div>
    `;
  });
...
// Evento para eliminar un producto del modal (botones "Eliminar")
$(document).on('click', '.btn-eliminar-producto', function() {
  $(this).closest('.grupo-producto').remove();
});

// Crear un nuevo bloque producto vacío para el modal
...
// Agregar producto nuevo al hacer click en el botón "Agregar Producto"
